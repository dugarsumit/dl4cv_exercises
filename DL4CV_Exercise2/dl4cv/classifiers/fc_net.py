...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out_affine_l2, cache_affine_l2 = affine_forward(out_l1, W2, b2)
        scores = out_affine_l2
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        ############################################################################
        pass
        loss, dout_l2 = softmax_loss(scores, y)
        # calculate regularized loss
        loss = loss + 0.5 * self.reg * (np.sum(W1 * W1) + np.sum(W2 * W2))
        dout_l1, dW2, db2 = affine_backward(dout_l2, cache_affine_l2)
        dout_input, dW1, db1 = affine_relu_backward(dout_l1, cache_l1)
